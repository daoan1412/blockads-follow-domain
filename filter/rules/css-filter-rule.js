...
var CssFilterRule = (function () {
    ...
    var convertCssInjectionRule = function (pseudoClass, cssContent) {
        ...
        if (utils.strings.isEmpty(selector) || utils.strings.isEmpty(style)) {
    ...
    var parsePseudoClass = function (selector) {
        ...
        var nameEndIndex = utils.strings.indexOfAny(selector, [' ', '\t', '>', '(', '[', '.', '#', ':', '+', '~', '"', "'"], nameStartIndex + 1);
        if (nameEndIndex < 0) {
    ...
    var parseMask = function (rule, isExtendedCss, isInjectRule) {
        ...
        if (!isExtendedCss) {
            if (isInjectRule) {
                isException = utils.strings.contains(rule, FilterRule.MASK_CSS_EXCEPTION_INJECT_RULE);
                mask = isException ? FilterRule.MASK_CSS_EXCEPTION_INJECT_RULE : FilterRule.MASK_CSS_INJECT_RULE;
            } else {
                isException = utils.strings.contains(rule, FilterRule.MASK_CSS_EXCEPTION_RULE);
                mask = isException ? FilterRule.MASK_CSS_EXCEPTION_RULE : FilterRule.MASK_CSS_RULE;
            }
        } else {
            if (isInjectRule) {
                isException = utils.strings.contains(rule, FilterRule.MASK_CSS_EXCEPTION_INJECT_EXTENDED_CSS_RULE);
                mask = isException ? FilterRule.MASK_CSS_EXCEPTION_INJECT_EXTENDED_CSS_RULE : FilterRule.MASK_CSS_INJECT_EXTENDED_CSS_RULE;
            } else {
                isException = utils.strings.contains(rule, FilterRule.MASK_CSS_EXCEPTION_EXTENDED_CSS_RULE);
                mask = isException ? FilterRule.MASK_CSS_EXCEPTION_EXTENDED_CSS_RULE : FilterRule.MASK_CSS_EXTENDED_CSS_RULE;
            }
        }
    ...
    var constructor = function (rule, filterId) {
        ...
        var mask = FilterRule.findRuleMarker(rule, CssFilterRule.RULE_MARKERS, CssFilterRule.RULE_MARKER_FIRST_CHAR);
        if (!mask) {
...
CssFilterRule.WHITELIST_MARKERS = [
    FilterRule.MASK_CSS_EXCEPTION_RULE, FilterRule.MASK_CSS_EXCEPTION_INJECT_RULE,
    FilterRule.MASK_CSS_EXCEPTION_EXTENDED_CSS_RULE, FilterRule.MASK_CSS_EXCEPTION_INJECT_EXTENDED_CSS_RULE];

/**
 * Masks indicating extended css rules
 */
CssFilterRule.EXTCSS_MARKERS = [
    FilterRule.MASK_CSS_EXCEPTION_INJECT_EXTENDED_CSS_RULE, FilterRule.MASK_CSS_INJECT_EXTENDED_CSS_RULE,
    FilterRule.MASK_CSS_EXCEPTION_EXTENDED_CSS_RULE, FilterRule.MASK_CSS_EXTENDED_CSS_RULE];
...
CssFilterRule.INJECT_MARKERS = [
    FilterRule.MASK_CSS_EXCEPTION_INJECT_EXTENDED_CSS_RULE, FilterRule.MASK_CSS_INJECT_EXTENDED_CSS_RULE,
    FilterRule.MASK_CSS_EXCEPTION_INJECT_RULE, FilterRule.MASK_CSS_INJECT_RULE];


module.exports = CssFilterRule;
