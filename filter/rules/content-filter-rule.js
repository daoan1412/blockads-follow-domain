...
function Wildcard(pattern) {
  ...
  function wildcardToRegex(pattern) {
    ...
    pattern = utils.strings.replaceAll(pattern, "\\*", "[\\s\\S]*");
    pattern = utils.strings.replaceAll(pattern, "\\?", ".");
    return "^" + pattern + "$";
  }
  ...
  function extractShortcut(pattern) {
    var wildcardChars = ["*", "?"];
    var startIndex = 0;
    var endIndex = utils.strings.indexOfAny(pattern, wildcardChars);
    ...
    while (endIndex >= 0) {
      ...
      endIndex = utils.strings.indexOfAny(
...
function getQuoteIndex(text, startIndex) {
  ...
  while (nextChar === '"') {
    ...
    nextChar =
      text.length === quoteIndex + 1 ? "0" : text.charAt(quoteIndex + 1);
  }
...
var ContentFilterRule = function(ruleText, filterId) {
  FilterRule.call(this, ruleText, filterId);

  this.parentSearchLevel = DEFAULT_PARENT_SEARCH_LEVEL;
  this.maxLength = 8192;

  var mask = FilterRule.MASK_CONTENT_EXCEPTION_RULE;
  var indexOfMask = ruleText.indexOf(mask);
  if (indexOfMask >= 0) {
    this.whiteListRule = true;
  } else {
    mask = FilterRule.MASK_CONTENT_RULE;
  ...
  if (ruleStartIndex === -1) {
    ...
    this.tagName = ruleText.substring(
      indexOfMask + mask.length,
      ruleStartIndex
    );
  }
  ...
  while (ruleStartIndex !== -1) {
    var equalityIndex = ruleText.indexOf(FilterRule.EQUAL, ruleStartIndex + 1);
    ...
    attributeValue = utils.strings.replaceAll(attributeValue, '""', '"');
...
ContentFilterRule.prototype = Object.create(FilterRule.prototype);
...
module.exports = { ContentFilterRule, Wildcard };
