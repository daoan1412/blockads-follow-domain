...
const createRule = require('./filter-rule-builder');
...
var SafariContentBlockerConverter = (function () {
    ...
    var AGRuleConverter = (function () {
        ...
        var parseDomains = function (rule, included, excluded) {
            ...
            if (rule.permittedDomain) {
                domain = utils.url.toPunyCode(rule.permittedDomain.toLowerCase());
                ...
                while (iDomains--) {
                    if (domains[iDomains] !== "") {
                        ...
                        domain = utils.url.toPunyCode(domain.toLowerCase());
                        included.push(domain);
                    }
                }
            }
            ...
            if (rule.restrictedDomain) {
                domain = utils.url.toPunyCode(rule.restrictedDomain.toLowerCase());
        ...
        var addResourceType = function (rule, result) {
            ...
            var contentTypes = UrlFilterRule.contentTypes;
        ...
        var createUrlFilterString = function (filter) {
            ...
            var isWebSocket = (filter.permittedContentType === UrlFilterRule.contentTypes.WEBSOCKET);
        ...
        var checkWhiteListExceptions = function (rule, result) {
            ...
            function isUrlBlockRule(r) {
                return isSingleOption(r, UrlFilterRule.options.URLBLOCK) ||
                    isSingleOption(r, UrlFilterRule.options.GENERICBLOCK);
            }

            function isCssExceptionRule(r) {
                return isSingleOption(r, UrlFilterRule.options.GENERICHIDE) ||
                    isSingleOption(r, UrlFilterRule.options.ELEMHIDE);
            }
            ...
            if (rule.whiteListRule && rule.whiteListRule === true) {
                ...
                if (documentRule || isUrlBlockRule(rule) || isCssExceptionRule(rule)) {
                    ...
                    if (parseDomainResult !== null &&
                        ...
                        console.debug('Whitelist special warning for rule: ' + rule.ruleText);

                        return;
                    }

                    if (parseDomainResult === null || parseDomainResult.domain === null) {
                        console.debug('Error parse domain from rule: ' + rule.ruleText);
                        return;
                    }
    ...
    var printVersionMessage = function () {
        console.info('Safari Content Blocker Converter v' + CONVERTER_VERSION);
    };
    ...
    var parseAGRule = function (ruleText, errors) {
        try {
            ...
            var agRule = createRule(ruleText);
            if (agRule === null) {
    ...
    var applyCssExceptions = function (cssBlocking, cssExceptions) {
        console.info('Applying ' + cssExceptions.length + ' css exceptions');
        ...
        cssBlocking.forEach(function (r) {
            if (r.trigger["if-domain"] && (r.trigger["if-domain"].length > 0) &&
                ...
                console.debug('Safari does not support permitted and restricted domains in one rule');
        ...
        console.info('Css exceptions applied: ' + exceptionsAppliedCount);
        console.info('Css exceptions errors: ' + exceptionsErrorsCount);
        return result;
    };
    ...
    var compactCssRules = function (cssBlocking) {
        console.info('Trying to compact ' + cssBlocking.length + ' elemhide rules');
        ...
        console.info('Compacted result: wide=' + cssBlockingWide.length + ' domainSensitive=' + cssBlockingDomainSensitive.length);
        return {
    ...
    var convertLines = function (rules, optimize) {
        console.info('Converting ' + rules.length + ' rules. Optimize=' + optimize);
        ...
        for (var i = 0, len = agRules.length; i < len; i++) {
            ...
            if (badFilterExceptions.indexOf(agRule.ruleText) >= 0) {
                ...
                console.info('Rule ' + agRule.ruleText + ' removed with a $badfilter modifier');
                continue;
            }
            ...
            if (item !== null && item !== '') {
                ...
                if (item.action.type === 'block') {
                    ...
                    AGRuleConverter.isSingleOption(agRule, UrlFilterRule.options.GENERICHIDE)) {
                    contentBlocker.cssBlockingGenericHideExceptions.push(item);
                } else if (item.action.type === 'ignore-previous-rules' &&
                    AGRuleConverter.isSingleOption(agRule, UrlFilterRule.options.ELEMHIDE)) {
                    // elemhide rules
    ...
    var createConversionResult = function (contentBlocker, limit) {
        ...
        console.info('Content blocker length: ' + converted.length);
    ...
    var convertArray = function (rules, limit, optimize) {
        ...
        var regexConfiguration = SimpleRegex.regexConfiguration;
        ...
        try {
            ...
            if (rules === null) {
                console.error('Invalid argument rules');
                return null;
            }

            if (rules.length === 0) {
                console.info('No rules presented for convertation');
                return null;
            }
...
module.exports = SafariContentBlockerConverter;