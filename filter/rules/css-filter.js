...
CssFilter.prototype = {
  ...
  buildCss: function(domainName, options) {
    ...
    if (!genericHide && retrieveTraditionalCss) {
      ...
      if (cssInjectionOnly) {
        commonCss = this._buildCssByRules(
          this.commonRules.filter(function(rule) {
  ...
  _applyExceptionRules: function() {
    ...
    var exceptionRulesMap = this._arrayToMap(
      this.exceptionRules,
      "cssSelector"
    );
    ...
    var newDomainSensitiveRulesMap = this._arrayToMap(
      newDomainSensitiveRules,
      "ruleText"
    );

    this.domainSensitiveRules = this.domainSensitiveRules.concat(
      newDomainSensitiveRules
    );
  ...
  _addMarkerToInjectRule: function(rule) {
    var INJECT_HIT_START = " content: 'e1412";
    ...
    // check semicolon
    var ruleTextWithSemicolon = utils.strings.endsWith(
