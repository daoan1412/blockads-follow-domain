...
var isContentBlockerEnabled = false;
...
function getAsciiDomainRule(ruleText) {
  try {
    ...
    return utils.strings.replaceAll(
      ruleText,
      domain,
      utils.url.toPunyCode(domain)
    );
  } catch (ex) {
    console.error("Error getAsciiDomainRule from {0}, cause {1}", ruleText, ex);
    return "";
  }
}
...
function parseRuleDomain(ruleText, parseOptions) {
  try {
    ...
    for (i = 0; i < startsWith.length; i++) {
      ...
      if (utils.strings.startWith(ruleText, start)) {
    ...
    console.error("Error parsing domain from {0}, cause {1}", ruleText, ex);
    return null;
  }
}
...
function parseRuleText(ruleText) {
  ...
  if (utils.strings.startWith(urlRuleText, FilterRule.MASK_WHITE_LIST)) {
    startIndex = FilterRule.MASK_WHITE_LIST.length;
  ...
  if (
    utils.strings.startWith(urlRuleText, UrlFilterRule.MASK_REGEX_RULE) &&
    utils.strings.endsWith(urlRuleText, UrlFilterRule.MASK_REGEX_RULE) &&
    !utils.strings.contains(urlRuleText, UrlFilterRule.REPLACE_OPTION + "=")
  ) {
    parseOptions = false;
  }
        ...
        if (i > 0 && ruleText.charAt(i - 1) === ESCAPE_CHARACTER) {
          ...
          if (foundEscaped) {
            ...
            options = options.replace(
              ESCAPE_CHARACTER + UrlFilterRule.OPTIONS_DELIMITER,
...
function validateCspRule(rule) {
  ...
  if (rule.cspDirective) {
    ...
    if (
      cspDirective.indexOf("report-uri") >= 0 ||
...
function tryConvertToCspRule(rule, urlRuleText) {
  ...
  if (
    urlRuleText.indexOf("data:") === 0 ||
    urlRuleText.indexOf("|data:") === 0 ||
    urlRuleText.indexOf("blob:") === 0 ||
    urlRuleText.indexOf("|blob:") === 0
  ) {
...
function ReplaceOption(option) {
  var parts = utils.strings.splitByDelimiterWithEscapeCharacter(
    option,
    "/",
    ESCAPE_CHARACTER,
    true
...
function containsOption(options, option) {
  return options !== null && (options & option) === option; // jshint ignore:line
}
...
var UrlFilterRule = function(rule, filterId) {
  ...
  if (this.isRegexRule) {
    this.urlRegExpSource = urlRuleText.substring(
      UrlFilterRule.MASK_REGEX_RULE.length,
      urlRuleText.length - UrlFilterRule.MASK_REGEX_RULE.length
    );
    ...
    if (
      UrlFilterRule.REGEXP_ANY_SYMBOL === regexp &&
      !this.hasPermittedDomains()
    ) {
...
UrlFilterRule.prototype.getUrlRegExp = function() {
  ...
  if (!this.urlRegExp) {
    ...
    try {
      if (
        !urlRegExpSource ||
        UrlFilterRule.MASK_ANY_SYMBOL === urlRegExpSource
      ) {
        // Match any symbol
      ...
      //malformed regexp
      console.error("Error create regexp from {0}", urlRegExpSource);
...
UrlFilterRule.prototype.isFiltered = function(
  ...
  if (
    this.shortcut !== null &&
    requestUrl.toLowerCase().indexOf(this.shortcut) < 0
  ) {
    return false;
  }
...
UrlFilterRule.prototype.isCheckThirdParty = function() {
  return (
    this.isOptionEnabled(UrlFilterRule.options.THIRD_PARTY) ||
...
UrlFilterRule.prototype._loadOptions = function(options) {
  var optionsParts = utils.strings.splitByDelimiterWithEscapeCharacter(
    options,
  ...
  for (var i = 0; i < optionsParts.length; i++) {
    ...
    switch (optionName) {
      ...
      case UrlFilterRule.IMPORTANT_OPTION:
        ...
        break;
      case FilterRule.NOT_MARK + UrlFilterRule.IMPORTANT_OPTION:
      ...
      case UrlFilterRule.EMPTY_OPTION:
        this._setUrlFilterRuleOption(
          UrlFilterRule.options.EMPTY_RESPONSE,
          true
        );
        break;
      ...
      case UrlFilterRule.REPLACE_OPTION:
        ...
        if (this.whiteListRule) {
          throw "Replace modifier cannot be applied to a whitelist rule " +
            this.ruleText;
        }
        if (optionsKeyValue.length > 1) {
          var replaceOption = optionsKeyValue[1];
          if (optionsKeyValue.length > 2) {
            replaceOption = optionsKeyValue.slice(1).join(FilterRule.EQUAL);
          }
      ...
      case UrlFilterRule.BADFILTER_OPTION:
        this.badFilter = this.ruleText
          .replace(
            UrlFilterRule.OPTIONS_DELIMITER +
              UrlFilterRule.BADFILTER_OPTION +
              FilterRule.COMA_DELIMITER,
            UrlFilterRule.OPTIONS_DELIMITER
          )
          .replace(
            FilterRule.COMA_DELIMITER + UrlFilterRule.BADFILTER_OPTION,
            ""
          )
          .replace(
            UrlFilterRule.OPTIONS_DELIMITER + UrlFilterRule.BADFILTER_OPTION,
            ""
          );
        break;
      default:
        ...
        if (optionName in UrlFilterRule.contentTypes) {
          this._appendPermittedContentType(
            UrlFilterRule.contentTypes[optionName]
          );
        } else if (
          optionName[0] === FilterRule.NOT_MARK &&
          optionName.substring(1) in UrlFilterRule.contentTypes
        ) {
          this._appendRestrictedContentType(
            UrlFilterRule.contentTypes[optionName.substring(1)]
          );
  ...
  if (
    this.isOptionEnabled(UrlFilterRule.options.JSINJECT) ||
...
UrlFilterRule.prototype._setUrlFilterRuleOption = function(option, enabled) {
  if (enabled) {
    if (
      (this.whiteListRule &&
        containsOption(UrlFilterRule.options.BLACKLIST_OPTIONS, option)) ||
      (!this.whiteListRule &&
        containsOption(UrlFilterRule.options.WHITELIST_OPTIONS, option))
    ) {
...
if (true) {
  UrlFilterRule.contentTypes.OBJECT_SUBREQUEST =
    UrlFilterRule.contentTypes.OBJECT;
}
...
UrlFilterRule.options.DOCUMENT_WHITELIST =
  ...
  UrlFilterRule.options.JSINJECT |
  UrlFilterRule.options.CONTENT; // jshint ignore:line
