...
FilterRule.prototype = {
  ...
  loadDomains: function(domains) {
    if (utils.strings.isEmpty(domains)) {
      return;
    }
    ...
    try {
      for (var i = 0; i < parts.length; i++) {
        var domain = parts[i],
          domainName;
        if (utils.strings.startWith(domain, "~")) {
          domainName = utils.url.toPunyCode(domain.substring(1).trim());
          if (!utils.strings.isEmpty(domainName)) {
          ...
          domainName = utils.url.toPunyCode(domain.trim());
          if (!utils.strings.isEmpty(domainName)) {
      ...
      console.error("Error load domains from {0}, cause {1}", domains, ex);
    }
  ...
  isPermitted: function(domainName) {
    ...
    if (
      this.restrictedDomain &&
      utils.url.isDomainOrSubDomain(domainName, this.restrictedDomain)
    ) {
      return false;
    }
    ...
    if (
      ...
      utils.url.isDomainOrSubDomainOfAny(domainName, this.restrictedDomains)
    ) {
      return false;
    }
    ...
    if (this.hasPermittedDomains()) {
      if (
        this.permittedDomain &&
        utils.url.isDomainOrSubDomain(domainName, this.permittedDomain)
      ) {
        return true;
      }

      return utils.url.isDomainOrSubDomainOfAny(
        domainName,
  ...
  addRestrictedDomains: function(domains) {
    if (domains) {
      ...
      restrictedDomains = utils.collections.removeDuplicates(
  ...
  removeRestrictedDomains: function(domains) {
    if (domains) {
      ...
      for (var i = 0; i < domains.length; i++) {
        utils.collections.remove(restrictedDomains, domains[i]);
      }
      this.setRestrictedDomains(restrictedDomains);
    }
  }
...
FilterRule.escapeRule = function(ruleText) {
  return encodeURIComponent(ruleText).replace(/['()]/g, function(match) {
    return { "'": "%27", "(": "%28", ")": "%29" }[match];
  });
};
